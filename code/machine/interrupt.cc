...
PendingInterrupt::PendingInterrupt(VoidFunctionPtr func, int param, int64_t time, 
				IntType kind)
{
    handler = func;
    arg = param;
...
// advance simulated time
    ...
    {
        stats->totalTicks += (int64_t)SystemTick;
	    stats->systemTicks += (int64_t)SystemTick;
    } else 
    {					// USER_PROGRAM
	    stats->totalTicks += (int64_t)UserTick;
	    stats->userTicks += (int64_t)UserTick;
    }
...
//----------------------------------------------------------------------
void
Interrupt::Schedule(VoidFunctionPtr handler, int arg, int64_t fromNow, IntType type)
{
    int64_t when = stats->totalTicks + fromNow;
    ...
    if(DebugIsEnabled('i'))
        cout << "Scheduling interrupt handler the " << intTypeNames[type] << " at time = " << hex << when << endl;
    ASSERT(fromNow > 0);
...
// Check if there is nothing more to do, and if so, quit
    ...
    if(DebugIsEnabled('i'))
        cout << "Invoking interrupt handler for the " << intTypeNames[toOccur->type] << " at time " << hex << toOccur->when << endl;
#ifdef USER_PROGRAM
    if (machine != NULL)
...
PrintPending(int arg)
{
    ...
    cout << "Interrupt handler " << intTypeNames[pend->type] << ", scheduled at " << hex << pend->when << endl;
    //printf("Interrupt handler %s, scheduled at %llX\n", intTypeNames[pend->type], pend->when);
}
...
Interrupt::DumpState()
{
    ...
    cout << "Time: 0x" << hex << stats->totalTicks << ", interrupts " << intLevelNames[level] << endl;
